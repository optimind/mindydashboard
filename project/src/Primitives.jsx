// Primitives.jsx — Button, Chip, Avatar, Icon
const { useState } = React;

function Icon({ name, size = 20, stroke = 2, style = {} }) {
  // Inline Lucide-style SVGs for the icons we use
  const paths = {
...
function Button({ variant = 'primary', size = 'md', children, icon, onClick, disabled, style = {} }) {
  const variants = {
...
function Chip({ variant = 'info', children, dot = true }) {
  const v = {
...
function Avatar({ src, name, size = 36, status, bob = false }) {
  const style = {
...
Object.assign(window, { Icon, Button, Chip, Avatar });
